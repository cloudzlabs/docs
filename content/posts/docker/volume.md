...
Container의 Writable Layer에 Data를 저장할 수는 있지만, 몇 가지 단점이 존재합니다.

- Container가 기동이 중지되면 Data도 보존되지 않습니다. 또한, 다른 프로세스에서 Container에 저장된 Data를 사용하기 어렵습니다.
- Container의 Writable Layer는 Container가 실행 중인 Host Machine과 밀접하게 연결됩니다. 따라서, Data를 다른 곳으로 쉽게 옮길 수 없습니다.
- Container의 Writable Layer에 Data를 저장하기 위해서는 File System을 관리하는 Storage Driver가 필요합니다. Storage Driver는 Linux 커널을 사용하여 공용 File System을 제공합니다. 이 기능은 Host File System에 직접 쓰는 `data volume`보다 성능이 떨어집니다.

Docker는 Docker Host에 Data를 Mount하는 방식으로 `volume, bind mounts, tmpfs`의 3가지 방식을 제공합니다(어떤 것을 사용해야할 지 모를 때는 `volume`를 사용하시기 바랍니다). 아래는 Container의 Data 관리를 방식들과 사용 사례에 대해서 자세히 살펴보도록 하겠습니다.
...
- `volume`는 Docker(Linux에서는 `/var/lib/docker/volume/`)가 관리하는 Host File System의 일부에 저장됩니다.
...
- Docker Host 또는 Docker Container의 Non-Docker 프로세서들이 언제든지 수정할 수 있습니다.
...
- Docker가 생성하고 관리하는 방식입니다. `docker volume create` 명령을 사용하여 명시적으로 voluems를 생성하거나, Container나 Service 생성 중에 volume를 생성할 수 있습니다.
- `volume`가 생성되면, Data는 Docker Host의 디렉토리에 저장됩니다. 해당 `volume`를 Container에 Mount하면, Host의 디렉토리가 Mount가 됩니다. 이는 `volume`가 Docker에 의해 관리되고 Host System의 핵심기능과 분리된다는 점을 제외한다면, `bind mount`와 유사하게 동작합니다.
- 주어진 `volume`를 동시에 여러 Container에 Mount할 수 있습니다.
- 실행 중인 Container가 `volume`를 사용하지 않아도, 해당 `volume`는 Docker에서 계속 사용할 수 있으며 자동으로 삭제되지 않습니다.
- `docker volume prune`을 사용하여 사용하지 않는 `volume`를 정리할 수 있습니다.
- `volume`을 Mount할 때, 이름을 명시적으로 지정하여 사용할 수 있으며, 익명으로도 사용할 수 있습니다.
- 익명의 `volume`는 처음 Mount될 때, 명시적으로 이름이 부여되지 않기 때문에, Docker는 주어진 Docker Host내에서 고유한 임의의 이름을 해당 `volume`에 부여합니다.
- 이름이 지정된 여부와 상관 없이 `volume`는 동일한 방식으로 작동합니다.
- 또한, `volume`는 원격 Host와 Cloud Provider가 Data를 저장할 수 있는 `volume drivers` 사용을 지원하고 있습니다.
...
- 비영구 상태 정보 또는 민감한 정보들같이 Container의 생명주기와 맞춰서 Data들을 보존해야할 때 사용할 수 있습니다.
- 예를 들어, Swarm Service는 내부적으로 `tmps mount`를 사용하여 Secret 정보를 Service의 Container에 Mount하여 사용합니다.

bind mount 및 volume는 -v 또는 --volume Flag를 사용하여, Container에 모두 Mount할 수 있지만 각 구문은 약간 차이가 있습니다. tmpfs mount의 경우, --tmpfs Flag를 사용합니다. 그러나 Docker 17.06 이상에서는 bind mount, volume, tmpfs mount에 대해 Container와 Service 모두에 --mount Flag를 사용하는 것이 구문이 더 명확이기 때문에 권장합니다.

## Mount 유형별 사용 사례

### volume 사용 사례

`volume`는 Docker Container 및 Service에서 Data를 유지하는 기본적인 방법입니다.

다음과 같은 경우에 `volume`를 사용합니다.

- **실행 중인 여러 Container 간에 Data 공유가 필요한 경우**
  - `volume` 생성에 대해 명시하지 않을 경우, Container에  처음으로 Mount될 때 생성됩니다.
  - Container가 중지되거나 제거되더라도 `volume`는 유지됩니다.
  - 여러 Container들이 동일한 `volume`을 읽기/쓰기 또는 읽기 전용으로 동시에 Mount할 수 있습니다.
  - `volume`는 명시적으로 선언하여 제거합니다.
- **Docker Host가 특정 디렉토리나 파일 구조를 가질 수 없는 경우**
  - `volume`는 Container Runtime으로부터 Docker Host의 설정을 분리하는데 도움이 됩니다.
- **로컬외의 원격 Host 및 Cloud Provider에 Container의 Data를 저장하려는 경우**
  - 한 Docker Host에서 다른 Docker Host로 Data를 백업이나 복구 또는 마이그레이션을 하는 경우 volume을 사용하면 더 좋습니다.
  - `volume`을 사용하여 Container를 중지한 다음 `volume`의 디렉토리(예: `/var/lib/docker/volume/&lt;volume-name&gt;`)을 백업할 수 있습니다.

### bind mount 사용 사례

일반적인 경우, 가능하면 `bind mount`보다 `volume`를 사용하십시오.
...
`tmpfs mount`는 Host System이나 Container 내에서 Data를 유지하지않아야할 때 가장 적합합니다. 이는 보안상의 이유가 될 수도 있고, Application이 수 많은 양의 비영구적인 상태의 Data를 작성해야 할 때 Container의 성능을 보호하기 위한 것일 수도 있습니다.

## volume 또는 bind mounts 사용 시, 유의사항

`volume` 또는 `bind mount`를 사용하는 경우, 다음의 사항에 대해 유의하시기 바랍니다.

- 파일 또는 디렉토리가 있는 Container내의 디렉토리에 빈 `volume`를 Mount하면, 해당 파일들 또는 디렉토리들이 `volume`로 전달(복사)됩니다. 마찬가지로, 아직 존재하지 않는 특정 `volume`을 지정하고 Container를 시작하면, 빈 `volume`가 생성됩니다. 이는 다른 Container에 필요한 Data를 미리 갖출 수 있는 좋은 방법입니다.
- 파일 또는 디렉토리가 있는 Container내의 디렉토리에 bind mount 또는 비어있지 않은 volume를 Mount한다면, Linux Host에서 파일들을 /mnt에 저장하고 나서 USB 드라이브가 /mnt에 Mount되는 것처럼 파일들 또는 디렉토리들이 Mount에 의해 가려지게 됩니다. /mnt의 컨텐츠들은 USB 드라이브가 Unmount될 때까지, USB 드라이브의 컨텐츠들에 의해서 가려지게 됩니다. 가려진 파일들은 제거되거나 대체된 것은 아니지만, bind mount 또는 volume가 Mount되어 있는 동안은 접근할 수 없습니다.
...
- `-v` 또는 `--volume`
  - 콜론(:)로 구분된 세 개의 필드로 구성되어있습니다. 필드의 순서가 정확해야 하며, 각 필드의 의미는 명확하지 않습니다.
  - 이름이 명시된 `volume`의 경우, 첫번째 필드는 `volume`의 이름이며, Host Machine에서 유일해야합니다. 익명 `volume`의 경우, 첫번째 필드는 생략됩니다.
  - 두번째 필드는 파일 또는 디렉토리가 Container에 Mount될 경로입니다.
  - 세번째 필드는 선택사항이며, 쉼표(,)로 구분된 옵션 목록(예: `ro`(읽기전용을 나타내는 옵션))입니다.
- `--mount`
  - 쉼표(,)로 구분되고 각각 &lt;key&gt;=&lt;value&gt; 한벌로 구성된 여러 key-value 쌍으로 구성됩니다. `--mount` 구문은 `-v` 또는 `--volume`보다 길지만, key의 순서는 중요치 않으며 Flag의 값을 이해하기 쉽게 되어있습니다.
  - `type`은 `bind, volume, tmpfs`로 Mount 유형을 지정합니다.
  - `source`는 `volume`의 이름입니다. 익명 `volume`은 해당 필드는 생략됩니다. `source` 또는 `src`로 지정할 수 있습니다.
...
Docker Swarm Mode의 Service도 Container와 동일한 방식으로 `volume`을 사용할 수 있습니다. Service 시작하고 `volume`을 정의하면, 각 Service Container들은 자체 로컬 `volume`을 사용합니다. 로컬 `volume driver`를 사용하는 경우, Container들 중 어느 것도 해당 Data들을 공유할 수 없지만 일부
`volume driver`는 공유 Storage를 지원합니다. AWS용 Docker와 Azure용 Docker는 모두 Cloudstor 플러그인을 사용하여 영구 저장소를 지원합니다.

### Container로 volume에 Data 적재하기

Container를 시작할 때 새로운 `volume`을 생성하고, 해당 Container의 Mount할 디렉토리내에 파일 또는 디렉토리가 있으면(예: `/app/`) 디렉토리의 내용이 `volume`으로 복사됩니다. 그러면 Container는 **volume**을 Mount하여 사용하고, 다른 Container도 미리 채워진 `volume`내의 컨턴츠에 접근할 수 있습니다.
...
일부 Application 개발의 경우, Container가 Docker Host로 변경 사항을 다시 전달할 수 있도록 `bind mount`하여 쓰기가 가능해야 합니다. 반대로, 다른 Container들은 단지 Data를 읽기만 하여야 하고, 수정하지 못해야 합니다. 즉, 여러 Container가 동일한 `volume`을 Mount할 수 있으며, 일부 Container는 읽기/쓰기로, 그 외 Container는 읽기전용으로 Mount하여 사용할 수도 있습니다.
